...
class ConfigBert(object):
    def __init__(self, args):
        ...
        self.test_batch_size = 3 if self.dataset == 'dwie' else 20
    ...
    def get_test_batch(self):
        ...
        context_pos = torch.LongTensor(self.test_batch_size, self.max_length)
        ...
        context_ner = torch.LongTensor(self.test_batch_size, self.max_length)
        ...
        context_seg = torch.LongTensor(self.test_batch_size, self.max_length)
        ...
        node_position_sent =  torch.zeros(self.test_batch_size, self.max_sent_num, self.max_node_per_sent, self.max_sent_len).float()

        node_position =  torch.zeros(self.test_batch_size, self.max_node_num, self.max_length).float().cuda()
        entity_position =  torch.zeros(self.test_batch_size, self.max_entity_num, self.max_length).float().cuda()
        node_num = torch.zeros(self.test_batch_size, 1).long().cuda()

        node_sent_num =  torch.zeros(self.test_batch_size, self.max_sent_num).float().cuda()

        sdp_position = torch.zeros(self.test_batch_size, self.max_entity_num, self.max_length).float().cuda()
        sdp_num = torch.zeros(self.test_batch_size, 1).long().cuda()
    ...
    def train(self, model_pattern, model_name):
        ...
        other_params = [p for name, p in ori_model.named_parameters() if p.requires_grad and not name.startswith("bert") and not any(nd in name for nd in graph_layer)]
    ...
    def test(self, model, model_name, output=False, input_theta=-1):
        ...
        if not self.is_test:
            logging('ALL   : Theta {:3.4f} | F1 {:3.4f} | Precision {:3.4f} | Recall {:3.4f} | AUC {:3.4f} '.format(input_theta, f1, pr_x[f1_pos], pr_y[f1_pos], auc))
        else:

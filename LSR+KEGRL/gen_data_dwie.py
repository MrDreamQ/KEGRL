...
MAX_ENTITY_NUM = 100
MAX_SENT_NUM = 220
MAX_NODE_PER_SENT = 20

parser = argparse.ArgumentParser()
parser.add_argument('--in_path', type = str, default =  "../dataset/DWIE")
parser.add_argument('--out_path', type = str, default = "prepro_dwie_glove")
...
def GetNodePosition(data, node_position, node_position_sent, node_sent_num, entity_position, Ls):
    ...
    # 文档中每个句子的实体位置
    for ns_no, ns in enumerate(data['vertexSet']):  # for entity in entities
        for n in ns:    # for mention in entity
            sent_id = int(n['sent_id'])
            doc_pos_s =  n['absolute_pos'][0]
            doc_pos_e =  n['absolute_pos'][1]
            assert( doc_pos_e <= Ls[-1] )
            nodes[sent_id].append([sent_id]+[ns_no] + [doc_pos_s, doc_pos_e])
            if n['pos'][0] >= n['pos'][1]:  # 跨句pos及相同pos
                nodes_sent[sent_id].append([sent_id] + [n['pos'][0], n['pos'][0]+1])
            else:
                nodes_sent[sent_id].append([sent_id] + n['pos'])
    id = 0  # mention node count
...
def ExtractMDPNode(data, sdp_pos, sdp_num, Ls):
    ...
    for ns_no, ns in enumerate(data['vertexSet']):
        for n in ns:
            sent_id = int(n['sent_id'])
            if n['pos'][0] >= n['pos'][1]:  # 跨句pos及相同pos
                nodes[sent_id].append([sent_id] + [n['pos'][0], n['pos'][0]+1])
            else:
                nodes[sent_id].append([sent_id] + n['pos'])
...
def Init(data_file_name, rel2id, max_length = 1800, is_training = True, suffix=''):
...
def LSRAnalysis(data_file_name, split):
    ori_data = json.load(open(data_file_name))
    ...
    for i in range(len(ori_data)):
        ...
        nodes = [[] for _ in range(len(ori_data[i]['sents']))]
        nodes_sent = [[] for _ in range(len(ori_data[i]['sents']))]
        for ns_no, ns in enumerate(ori_data[i]['vertexSet']):  # for entity in entities
            for n in ns:    # for mention in entity
                sent_id = int(n['sent_id'])
        ...
        for sent_no, ns in enumerate(nodes_sent):
            #print("len of ns is {}".format(len(ns)))
            node_per_sent = len(ns)
            max_node_per_sent = max(max_node_per_sent, node_per_sent)
        ...
        node_num = id + entity_num
        max_node_num = max(max_node_num, node_num)
        
    print(f"{split} dataset max sentence num is {max_sent_num}")
    print(f"{split} dataset max node per sentence is {max_node_per_sent}")
    print(f"{split} dataset max node num is {max_node_num}")
    print(f"{split} dataset max sentence length is {max_sent_len}")
    print(f"{split} dataset max entity num is {max_entity_num}")

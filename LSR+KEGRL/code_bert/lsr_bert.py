...
SGRAPH_HIDDEN_DIM = 768
DGRAPH_HIDDEN_DIM = 768
...
class LSR(nn.Module):
    def __init__(self, config):
        ...
        if  config.sgraph or config.dgraph:
            kegrl_hidden_size = bert_hidden_size if config.dataset != 're' else hidden_size
            self.ra_enc = RaEncoder(config.relation_num - 1, kegrl_hidden_size, kegrl_hidden_size, layers_num=1)


        if config.sgraph:
            print(f"SGRAPH HIDDEN DIM: {SGRAPH_HIDDEN_DIM}")
            self.sgnn = GraphNetWork(kegrl_hidden_size, kegrl_hidden_size, 
                                    hidden_dim=SGRAPH_HIDDEN_DIM, output_dim=kegrl_hidden_size,
        ...
        if config.dgraph:
            ...
            self.dgnn = DoubleGraphNetWork(kegrl_hidden_size, kegrl_hidden_size,
                                    hidden_dim=DGRAPH_HIDDEN_DIM, 
                                    output_dim=kegrl_hidden_size,
        ...
        if self.config.sgraph or self.config.dgraph:
            self.entity_ensemble = nn.Linear(2 * bert_hidden_size, bert_hidden_size)
    ...
    def forward(self, context_idxs, pos, context_ner, h_mapping, t_mapping,
        ...
        max_doc_len = 512 if self.config.dataset != 'dwie' else 1800
        '''===========STEP1: Encode the document============='''
        if self.config.dataset != 'dwie':
            ...
            context_output = self.encode(context_idxs, context_masks)
            context_output = [layer[starts.nonzero().squeeze(1)]
                    for layer, starts in zip(context_output, context_starts)]
            context_output = pad_sequence(context_output, batch_first=True, padding_value=-1)
            context_output = torch.nn.functional.pad(context_output,(0,0,0,context_idxs.size(-1)-context_output.size(-2)))
        ...
        if self.config.sgraph and self.config.dataset != 're':
        ...
        if self.config.dgraph and self.config.dataset != 're':
        ...
        if self.config.sgraph and self.config.dataset == 're':
            ...
            hs = unbatch_entities(hs, entity_num_list.long().tolist())
            ts = unbatch_entities(ts, entity_num_list.long().tolist())
            start_re_output, end_re_output = get_ht(hs, ts, entity_num_list.long().tolist(), hts, hts_len)
        elif self.config.dgraph and self.config.dataset == 're':
            hs, ts, self.ada_loss = self.dgnn(graph_ht, graph_th, rel_feat, batch_entities(entities_emb, entity_num_list.long().tolist()))
            hs = unbatch_entities(hs, entity_num_list.long().tolist())
            ts = unbatch_entities(ts, entity_num_list.long().tolist())
            start_re_output, end_re_output = get_ht(hs, ts, entity_num_list.long().tolist(), hts, hts_len)
    ...
    def encode(self, input_ids, attention_mask):
        ...
        sequence_output = process_long_input(self.bert, input_ids, attention_mask, start_tokens, end_tokens)

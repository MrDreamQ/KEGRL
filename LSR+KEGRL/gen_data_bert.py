...
parser.add_argument('--in_path', type = str, default =  "../dataset/DocRED", choices=["../dataset/DocRED", "../dataset/Re-DocRED"])
parser.add_argument('--out_path', type = str, default = "prepro_bert", choices=["prepro_bert", "prepro_re_bert"])
...
if 'Re-DocRED' in in_path:
    train_annotated_file_name = os.path.join(in_path, 'train_revised.json')
    dev_file_name = os.path.join(in_path, 'dev_revised.json')
    test_file_name = os.path.join(in_path, 'test_revised.json')
else:
...
bert_config = BertConfig(max_position_embeddings=512)
bert = Bert(BertModel(config=bert_config), 'bert-base-uncased')

...
class Bert():
    ...
    def __init__(self, model_class, model_name, dataset='docred'):
        super().__init__()
        self.model_name = model_name
        self.tokenizer = BertTokenizer.from_pretrained(self.model_name)
        self.tokenizer.max_len = 512 if dataset == 'docred' else 1800
        self.tokenizer.max_len_sentences_pair = 509 if dataset == 'docred' else 1797
        self.tokenizer.max_len_single_sentence = 510 if dataset == 'docred' else 1798
    ...
    def subword_tokenize(self, tokens):
        ...
        sep = 509 if self.dataset == 'docred' else 1797
        subwords = [self.CLS] + list(self.flatten(subwords))[:sep] + [self.SEP]
        token_start_idxs = 1 + np.cumsum([0] + subword_lengths[:-1])
        token_start_idxs[token_start_idxs > sep] = sep

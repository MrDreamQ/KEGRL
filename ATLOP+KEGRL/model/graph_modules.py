...
class GraphNetWork(nn.Module):
    def __init__(self, rel_indim, entity_indim, rel_names, hidden_dim=1024, output_dim=1024, num_layers=2, adaptive=True):
        super().__init__()
    ...
    def forward(self, graphs, rel_embeds, entity_embeds):
        ...
        if self.adaptive:
            ada_loss = self.cal_ada_weight_loss(d_weight_ls)
        else:
            ada_loss = None
...
class MetaGraphNetWork(nn.Module):
    def __init__(self, rel_indim, entity_indim, rel_names, hidden_dim, num_layers=2, adaptive=True):
        super().__init__()
        self.adaptive = adaptive
        self.hidden_dim = hidden_dim
        self.rel_names = rel_names
...
class DoubleGraphNetWork(nn.Module):
    def __init__(self, rel_indim, entity_indim, rel_names_ht, rel_names_th, hidden_dim=1024, output_dim=128, num_layers=2, adaptive=True):
        super().__init__()
        self.adaptive = adaptive
        self.meta_graph_ht = MetaGraphNetWork(rel_indim, entity_indim, rel_names_ht, hidden_dim // 2, num_layers, adaptive)
        self.meta_graph_th = MetaGraphNetWork(rel_indim, entity_indim, rel_names_th, hidden_dim // 2, num_layers, adaptive)
